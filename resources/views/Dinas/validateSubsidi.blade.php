...
<body>
    <div class="flex flex-row">
        <div
            class="relative flex flex-col bg-clip-border rounded-xl bg-white text-gray-700 h-[100vh] w-full max-w-[20rem] p-4 shadow-xl shadow-gray-900/5">
            ...
            <nav class="flex flex-col gap-1 min-w-[240px] p-2 font-sans text-base font-normal text-gray-700">
                <div role="button" tabindex="0"
                    class="flex items-center w-full p-3 rounded-lg text-start leading-tight transition-all bg-[#F5682A]  text-white outline-none">
                    ...
                    </div><a href="/subsidiDinas">Pengajuan Pakan Subsidi</a>
        ...
        <div class="w-full bg-white rounded-lg shadow md:mt-0 xl:p-0">
            <div class="p-6 space-y-4 md:space-y-6 sm:p-8">
                ...
                <form class="space-y-4 md:space-y-6" action="{{ route('subsidi.validate', $foodSubmissions->id) }}"
                    method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    <div>
                        <label for="government_note" class="block mb-2 text-sm font-medium text-gray-900">Catatan
                            Dinas</label>
                        <input type="text" name="government_note" id="government_note" placeholder="Beri Catatan"
                    ...
                    <div class="flex flex-row gap-10">
                        <div class="flex flex-row items-center justify-center gap-6">
                            <div class="flex flex-col items-center justify-center">
                                <label for="Sudah"
                                    class="block mb-2 text-sm font-medium text-gray-900">Diterima</label>
                                <input type="radio" name="validation" id="Sudah" value="Diterima"
                                    @if ($foodSubmissions->validation == 'Diterima') checked @endif
                            ...
                            <div class="flex flex-col items-center justify-center">
                                <label for="Proses"
                                    class="block mb-2 text-sm font-medium text-gray-900">Diproses</label>
                                <input type="radio" name="validation" id="Proses" value="Diproses"
                                    @if ($foodSubmissions->validation == 'Diproses') checked @endif
                                    onclick="if(this.checked) {Sudah.checked=false;} {Belum.checked=false;}"
                                    class="bg-gray-50 border border-gray-500 text-gray-900 sm:text-sm rounded-lg focus:ring-[#F5682A] focus:border-[#F5682A] block p-2.5">
                            </div>
                            <div class="flex flex-col items-center justify-center">
                                <label for="Belum"
                                    class="block mb-2 text-sm font-medium text-gray-900">Ditolak</label>
                                <input type="radio" name="validation" id="Belum" value="Ditolak"
                                    @if ($foodSubmissions->validation == 'Ditolak') checked @endif
