            ...
            <form class="space-y-4 md:space-y-6" action="{{ route('update.berita', $broadcastBerita->id) }}" method="POST" enctype="multipart/form-data" novalidate>
                ...
                <div>
                    ...
                    class="overflow-hidden rounded-lg w-1/2 h-64 border border-gray-300 focus-within:border-[#F5682A] focus-within:ring-1 focus-within:ring-[#F5682A]">
                        <textarea id="isi" name="isi" class="p-2 w-full h-full resize-none border-none align-top focus:ring-0 sm:text-sm" placeholder="Tuliskan Isi Berita...">{{ $broadcastBerita->isi }}</textarea>
                    </div>
                    @error('isi')
                    <p class="text-red-500 text-sm font-medium py-2 capitalize" role="alert">{{ $message }}</p>
                    @enderror
